window.onload = function() {
    ...
    var sidelist = Box.getElementsByTagName("i");
    ...
    for(i=0;i<oLi.length;i++){
        oLi[i].index = i;
        oLi[i].onmouseover = function() { //鼠标移入侧边栏中  
            for(j=0;j<oLi.length;j++){   //遍历侧边栏 让其所有背景颜色变成灰色
        ...
        oLi[i].onmouseout = function() {   //移出时 让背景恢复为原来的背景  位移回原来的位置 动画效果1s
